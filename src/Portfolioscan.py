...
def _safe_last_value(df: pd.DataFrame, col: str):
...
def _rsi_trend_label(r: pd.Series, p: pd.Series):
    """Retourneert RSI richting als (label, sign)."""
    if r['RSI'] > p['RSI']:
        return "RSI ↑", 1
    elif r['RSI'] < p['RSI']:
        return "RSI ↓", -1
...
# ============================================================
#  CANDLE EN PATRON DETECTIE
# ============================================================

def _detect_candles(df: pd.DataFrame) -> list:
    """Detecteert bullish candles (Engulfing, Hammer)."""
    signals = []
    for i in range(-2, 0):
        idx = len(df) + i - 1
        if idx < 1:
...
def _detect_bearish_candles(df: pd.DataFrame) -> list:
    """Detecteert bearish candles (Engulfing, Shooting Star)."""
    signals = []
    for i in range(-2, 0):
        idx = len(df) + i - 1
        if idx < 1:
...
def _detect_divergence(df: pd.DataFrame, lookback: int = 30) -> str:
    ...
    curr_low_idx = lows
    if recent.loc[curr_low_idx, 'Low'] >= recent.loc[prev_low_idx, 'Low']:
        return ""
    rsi_curr = recent.loc[curr_low_idx, 'RSI']
    rsi_prev = recent.loc[prev_low_idx, 'RSI']
    if rsi_curr > rsi_prev:
        return f"RSI Bull Div (+{rsi_curr - rsi_prev:.1f})"
    macd_curr = recent.loc[curr_low_idx, 'Histogram']
    macd_prev = recent.loc[prev_low_idx, 'Histogram']
    if macd_curr > macd_prev:
        return f"MACD Bull Div (+{macd_curr - macd_prev:.3f})"
    return ""
...
def _detect_bearish_divergence(df: pd.DataFrame, lookback: int = 30) -> str:
    ...
    curr_high_idx = highs
    if recent.loc[curr_high_idx, 'High'] <= recent.loc[prev_high_idx, 'High']:
        return ""
    rsi_curr = recent.loc[curr_high_idx, 'RSI']
    rsi_prev = recent.loc[prev_high_idx, 'RSI']
    if rsi_curr < rsi_prev:
        return f"RSI Bear Div (-{rsi_prev - rsi_curr:.1f})"
    macd_curr = recent.loc[curr_high_idx, 'Histogram']
    macd_prev = recent.loc[prev_high_idx, 'Histogram']
    if macd_curr < macd_prev:
        return f"MACD Bear Div (-{macd_prev - macd_curr:.3f})"
    return ""
...
def _detect_volume_spike(df: pd.DataFrame, threshold: float = 3.0) -> str:
    ...
    if ratio > threshold and r['Close'] < r['SMA20']:
        return f"Vol Spike x{ratio:.1f}"
    return ""
...
def _detect_double_bottom(df: pd.DataFrame, window: int = 50, tolerance: float = 0.02) -> str:
    ...
    if recent.iloc[-1]['Close'] > neckline:
...
def assess_trend(df: pd.DataFrame, start_date: str, periods=[20,50,200]) -> tuple:
    ...
    if (r['Close'] > r['BB_Upper'] and r['MACD'] > r['Signal'] and
        r['Histogram'] > p['Histogram'] and r['RSI'] > 50 and
        r['RSI'] > p['RSI'] and vol_up):
        return "Trendswitch", df
    elif (r['Close'] > r['BB_Lower'] and r['MACD'] > r['Signal'] and
          40 < r['RSI'] < 60 and not vol_up):
        return "Rebound", df
    return "Uncertain", df

# ============================================================
#  HOOFDFUNCTIE TREND/BOUNCE SCORING
# ============================================================

def trend_bounce_score(df: pd.DataFrame, start_date: str = '2025-01-01',
    ...
    # === MOMENTUM ===
    momentum = []
    rsi_label, rsi_dir = _rsi_trend_label(r, p)
    momentum.append(f"{rsi_label} {r['RSI']:.0f}")
    macd_val, signal_val = r['MACD'], r['Signal']
    momentum.append(f"MACD {'↑' if macd_val > signal_val else '↓' if macd_val < signal_val else '→'} {macd_val:.3f}")
    vol_ratio = r['Volume'] / r['Vol_MA'] if r['Vol_MA'] > 0 else 1.0
    vol_sym = "↑" if vol_ratio > 1.1 else "↓" if vol_ratio < 0.9 else "→"
    momentum.append(f"Vol {vol_sym} {vol_ratio:.0%}")
    ...
    vol_high = r['Volume'] > r['Vol_MA'] * vol_high_mult
    vol_spike = r['Volume'] > r['Vol_MA'] * vol_spike_mult
    ...
    if (r['Close'] > r['BB_Lower'] and r['RSI'] > 30 and rsi_dir == 1 and vol_high and candles):
        long_score += 5
    ...
    vs = _detect_volume_spike(df_ind)
    if vs:
        long_score += 2
        long_details.append(vs)
    ...
    if vol_high:
        long_score += 1
        long_details.append(f"Vol +{vol_ratio:.0%}")
    long_score = min(long_score, 10)

    # --- SHORT RULES ---
    if (r['Close'] < r['BB_Lower'] and r['MACD'] < r['Signal'] and
        r['Histogram'] < p['Histogram'] and r['RSI'] < 50 and r['RSI'] < p['RSI'] and vol_high):
        short_score += 4
        short_details.append("Trendbreak")
    elif (r['Close'] < r['BB_Upper'] and r['MACD'] < r['Signal'] and 40 < r['RSI'] < 60 and not vol_high):
        short_score += 2
        short_details.append("Bearish Drift")
    if r['RSI'] < rsi_oversold:
        short_score += 3
        short_details.append(f"Oversold (RSI<{rsi_oversold:.0f})")
    if (r['Close'] < r['BB_Upper'] and r['RSI'] < 70 and vol_ratio > 1.1 and bear_candles):
        short_score += 5
        short_details.append(f"{bear_candles[0][0]} + RSI {r['RSI']:.1f}")
    bd = _detect_bearish_divergence(df_ind)
    if bd:
        short_score += 3
        short_details.append(bd)
    if vol_spike and r['Close'] > r['SMA20']:
        short_score += 2
    ...
    if hs:
        short_score += 5 if "→ BD" in hs else 3
        short_details.append(hs)
    if vol_high:
        short_score += 1
        short_details.append(f"Vol +{vol_ratio:.0%}")
    ...
    return {
        ...
        "long_details": " | ".join(long_details) or "Geen long",
        "short_score": short_score,
        "short_details": " | ".join(short_details) or "Geen short",
        "actie": actie,
...
def scan_portfolio(tickers: list, start_date: str = '2025-01-01', min_data_days: int = 50) -> pd.DataFrame:
    ...
    for t in tqdm(tickers):
        try:
            ...
            price = _safe_last_value(df, 'Close') or _safe_last_value(df, 'Adj Close')
    ...
    print("="*160)
    display(df_out[["ticker", "price", "long_score", "short_score", "actie", "momentum"]])

    koop = df_out[df_out["actie"].isin(["KOOP DIP", "KOOP VROEG"])]
    if not koop.empty:
        print("\n" + "KOOPKANSEN".center(80, " "))
        print("="*80)
        display(koop[["ticker", "price", "actie", "long_details", "momentum"]])

    verkoop = df_out[df_out["actie"].isin(["STERKE SHORT", "SHORT"])]
    if not verkoop.empty:
        print("\n" + "VERKOOPKANSEN".center(80, " "))
        print("="*80)
        display(verkoop[["ticker", "price", "actie", "short_details", "momentum"]])
